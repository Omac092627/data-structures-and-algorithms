...
const countNumberOfElements = (arr) => {
  let elements = arr.reduce( (accumulator, val, idx) => {
    accumulator++;
    return accumulator;
  });
  return elements;
...
const returnNames = (arr) => {
  let namesArr = arr.reduce((name, val, idx) => {
    name[idx] = val.name;
    return name;
  }, []);
  return namesArr;
...
const reversedString = (str) => {
  let arr = str.split('');
  let reversedStrn = arr.reduce((newString, currentLetter) => {
...
const calculateAverage = (arr) => {
  let avg= arr.reduce((avg, value, idx) => {
    avg = avg + value;
    return avg;
  }, 0);
...
const countPrimeNumbers = (arr) => {
  let primeNumbers = arr.reduce((num, val, idx) => {
    if(isPrime(val) === true){
      num++;
    }
    return num;
