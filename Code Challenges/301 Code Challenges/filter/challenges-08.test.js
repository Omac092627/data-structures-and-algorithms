...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let newArr = arr.filter(arr => arr.baseStat > minBaseStat);
  return newArr;
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  for (let i = 0; i < newArr.length; i++){
    nameArr.push(newArr[i].stat.name);
  }
  return nameArr;
};
...
const getCharactersWithoutChildren = (arr) => {
  let newArr = arr.filter(kids => !kids.children);
  return newArr;
};
...
const evenOddNumericValues = (arr) => {
  ...
  let stringArr = newArr.map( function(num){
    if(num % 2 === 0){
      return 'even';
    } else {
      return 'odd';
